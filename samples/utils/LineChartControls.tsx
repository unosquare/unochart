      ...
      <div className="p-4 bg-gray-50 rounded-lg">
        {lines.map((line, index) => (
          <div key={line.id} className="mb-4">
            <label className="block text-sm font-medium text-gray-700 mb-1">Line {index + 1} Stroke Color</label>
            <input
              type="color" value={line.stroke} onChange={(e) => handleLineChange(index, 'stroke', e.target.value)}
              className="w-8 h-8"
            />
            ...
            {hasNullValues(line.dataKey) && (
              <label className="flex items-center space-x-2 text-gray-700 mt-4">
                <input
                  type="checkbox" checked={line.connectNulls} onChange={(e) => handleLineChange(index, 'connectNulls', e.target.checked)}
